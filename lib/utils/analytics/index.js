...
const analyticsUrl = require('./url');
...
function request(payload, { id, timeout, url } = {}) {
  return BbPromise.try(() => {
    if (!url) url = analyticsUrl;
    if (!url) return null;
    ++ongoingRequestsCount;
...
function report(payload, options = {}) {
  return BbPromise.try(() => {
    ...
    if (!cacheDirPath) return request(payload, { url: options.url });
    const id = uuid();
    return BbPromise.all([
      ...
      request(payload, { id, url: options.url }),
