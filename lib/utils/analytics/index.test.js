...
describe('analytics', () => {
  ...
  const testOptions = { url: analyticsUrl };

  const sendReport = () => {
    return report({}, testOptions);
  };
  ...
  it('Should ignore missing cacheDirPath', () =>
    sendPending(testOptions).then(sendPendingResult => {
  ...
  it('Should cache failed requests and rerun then with sendPending', () => {
    ...
    return sendReport()
      ...
      .then(dirFilenames => {
        ...
        return sendPending(testOptions);
      })
  ...
  it('Should limit concurrent requests at sendPending', () => {
    ...
    return Promise.all([
      ...
      .then(dirFilenames => {
        ...
        return sendPending(Object.assign({ serverlessExecutionSpan }, testOptions));
      })
      .then(() => fse.readdir(cacheDirPath))
      .then(dirFilenames => {
        expect(dirFilenames.filter(isFilename).length).to.equal(0);
